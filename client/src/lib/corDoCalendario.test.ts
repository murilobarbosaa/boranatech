...
describe("MARCADOR_DA_COR", () => {
  it("tem exatamente as 7 chaves de CORES_DO_CALENDARIO", () => {
  ...
  it("cada valor tem o tom cheio da propria familia e a borda de tinta", () => {
    for (const cor of CORES_DO_CALENDARIO) {
      const classes = MARCADOR_DA_COR[cor];
      expect(classes, cor).toContain(`bg-${cor}-500`);

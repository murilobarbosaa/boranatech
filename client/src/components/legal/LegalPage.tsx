...
export default function LegalPage({
  ...
  return (
    <Layout>
      <section
        className={`relative overflow-hidden border-b-2 border-slate-900 ${tone.hero} py-12 md:py-16`}
      >
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
            ...
            >
              ...
              <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-600">
                Documento público para explicar regras, direitos e limites da
                plataforma.
              </p>
          ...
          <div className="mb-8 grid gap-4 md:grid-cols-3">
            {highlights.map((highlight) => (
              ...
              >
                ...
                <p className="mt-2 font-display text-xl font-black leading-snug text-slate-950">
                  {highlight.value}
                </p>
              </article>
            ))}
              ...
              <ul className="space-y-2">
                {sections.map((section) => (
                  <li key={section.id}>
                    <a
                      href={`#${section.id}`}
                      className="group flex items-center justify-between gap-3 rounded-xl px-3 py-2 text-sm font-bold text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-950"
                    >
              ...
              {sections.map((section) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <span
                      className={`mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border-2 border-slate-900 text-sm font-black text-slate-950 ${tone.marker}`}
                    >
                      {section.title.match(/^\d+/)?.[0] || "•"}
                    </span>
                    <div className="min-w-0">
                      <h2 className="font-display text-2xl font-black leading-tight text-slate-950">
                        {section.title}
                      </h2>
                      {section.body}
                      {section.items ? (
                        <ul className="mt-4 space-y-3">
                          {section.items.map((item) => (
                            ...
                            >
                              <span
                                className={`mt-2 h-2.5 w-2.5 shrink-0 rounded-full border border-slate-900 ${tone.marker}`}
                              />
                              <span>{item}</span>
                            </li>
                          ))}
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    {relatedLinks.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="rounded-2xl border-2 border-slate-900 bg-white p-4 transition-transform hover:-translate-y-0.5"
                      >

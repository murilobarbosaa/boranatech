...
import { isComplete, isSectionUnlocked, nodeProgress } from "@/lib/roadmapV2/progress";
...
const WALK_MS = Math.round((WALK_DELAY + WALK_STAGGER * (DOTS - 1) + DOT_DURATION) * 1000);
...
function bezier(a: Point, cp1: Point, cp2: Point, b: Point, t: number): Point {
  const u = 1 - t;
  return {
    x: u * u * u * a.x + 3 * u * u * t * cp1.x + 3 * u * t * t * cp2.x + t * t * t * b.x,
    y: u * u * u * a.y + 3 * u * u * t * cp1.y + 3 * u * t * t * cp2.y + t * t * t * b.y,
  };
}
...
const RoadmapTrail = forwardRef<TrailHandle, RoadmapTrailProps>(function RoadmapTrail(
  { sections, done, onOpenSection },
  ref,
) {
  const layout = useMemo(() => buildLayout(sections.length), [sections.length]);
  ...
  const arrivalTimers = useRef<Map<number, ReturnType<typeof setTimeout>>>(new Map());
  ...
  useImperativeHandle(ref, () => ({ burst, walk, unwalk }), [burst, walk, unwalk]);
    ...
    <div className="relative mt-7 w-full">
      ...
      >
        {layout.segments.map((dots, si) => (
          <motion.g
            ...
            variants={{
              lit: { transition: { staggerChildren: 0.26, delayChildren: 0.05 } },
              unlit: { transition: { staggerChildren: 0.04, staggerDirection: -1 } },
            }}
          >
            {dots.map((dot, k) => (
              <motion.circle
                ...
                r={6}
                variants={{
                  lit: { fill: "#FFB800", scale: [1, 2.1, 1], transition: { duration: 0.55, ease: "easeInOut" } },
                  unlit: { fill: "#d0d7e2", scale: 1, transition: { duration: 0.25 } },
                }}
      ...
      <div className="absolute inset-0">
        {sections.map((section, i) => {
          ...
          const prevHasNoRequired = i > 0 && nodeProgress(sections[i - 1], done).total === 0;
          const unlocked =

...
function searchAll(query: string): SearchResult[] {
  ...
  areasTI.forEach((a) => {
    if (
      ...
      a.descricaoCurta.toLowerCase().includes(q)
    ) {
      results.push({
        type: "Área da TI",
        title: a.nome,
        description: a.descricaoCurta,
        path: `/areas/${a.slug}`,
      });
    }
  });
...
export default function SearchBar({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      fetch(apiUrl(`/api/search?q=${encodeURIComponent(trimmed)}&limit=10`), {
        ...
        .then((json) => {
          const apiResults: SearchResult[] = (json.data || []).map(
            (item: any) => ({
        ...
        .catch((error) => {
          ...
          const fallbackResults = searchAll(trimmed);
          ...
          {results.length > 0 ? (
            <ul>
              {results.map((r, i) => (
                <li key={i}>
                  ...
                  >
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium mt-0.5 shrink-0 ${typeColors[r.type] || "bg-slate-100 text-slate-600"}`}
                    >
                      {r.type}
                    </span>
            ...
            <div className="px-4 py-6 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Tente buscar por "front-end", "Python" ou "roadmap"
              </p>
            </div>
          )}
        </div>

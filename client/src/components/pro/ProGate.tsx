...
export default function ProGate({ description, className = "" }: ProGateProps) {
  ...
  return (
    <div className={`card-brutal rounded-2xl border-slate-300 bg-slate-100 p-6 text-center ${className}`}>
      ...
      <h2 className="font-display text-2xl font-black text-slate-950">Disponível no Plano Pro</h2>
      <p className="mx-auto mt-2 max-w-2xl text-sm font-medium text-slate-600">{description}</p>
      <button

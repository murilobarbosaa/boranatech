...
export default function Roadmaps() {
  ...
  const visibleRoadmaps = isPro ? filtered : filtered.filter((r) => r.id === roadmapItems[0]?.id);
    ...
    <Layout>
      ...
      <section className="bg-emerald-100 py-12 border-b-2 border-slate-900 relative overflow-hidden">
        <div className="absolute inset-0 opacity-15">
          <img src={roadmapImage} alt="" className="w-full h-full object-cover" />
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-4">
            {roadmapPlans.map((plan) => (
              <button
                ...
                type="button"
                onClick={() => {
                  setDurationFilter(durationFilter === plan.days ? "todos" : plan.days);
                  setSelected(null);
                }}
            ...
            {isPro ? (
              <AiToolPanel
                ...
                fields={[
                  ...
                  { name: "goal", label: "Objetivo principal", placeholder: "Ex: montar portfólio, conseguir estágio, migrar de área" },
                  { name: "context", label: "Contexto extra", type: "textarea", placeholder: "Conte sua rotina, dificuldades, ferramentas que já conhece ou preferência de aprendizado." },
                ]}
              />
            ) : (
          ...
          <div className="lg:col-span-1 space-y-3">
            {visibleRoadmaps.length === 0 && (
              <div className="card-brutal rounded-xl border-dashed bg-white p-5 text-sm font-bold text-slate-600">
                Nenhum roadmap encontrado com esses filtros. Tente limpar a duração ou escolher outra área.
              </div>
            )}
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-black ${
                      selected === r.id ? "bg-white/15 text-white" : "bg-emerald-100 text-emerald-800"
                    }`}>
                      {r.duracaoDias}
                    </p>
                  </div>
            ...
            {selectedRoadmap ? (
              <div className="space-y-6">
                ...
                <div className="card-brutal bg-white rounded-xl p-6">
                  <h3 className="font-display font-bold text-xl text-slate-900 mb-6">Etapas da trilha</h3>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="card-brutal bg-red-50 rounded-xl p-5 border-red-200">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="font-semibold text-slate-900 text-sm">Erros comuns</h3>
                    </div>
              ...
              <div className="card-brutal bg-slate-50 rounded-xl p-12 text-center border-dashed">
                ...
                <h3 className="font-display font-bold text-xl text-slate-900 mb-2">Selecione um roadmap</h3>
                <p className="text-slate-600 text-sm">Clique em um roadmap à esquerda para ver a trilha completa.</p>
              </div>
            )}
          </div>
        </div>

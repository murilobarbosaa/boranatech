        ...
        <div className="container space-y-10">
          <div>
            <div className="mb-5">
              ...
              <h2 className="font-display text-3xl font-black text-slate-950">
                Do primeiro estágio ou trainee à liderança
              </h2>
              <p className="mt-2 max-w-3xl text-sm font-semibold text-slate-600">
                Cada etapa pede evidências diferentes. Para sair de estudante
                para estágio ou trainee, o mais importante é provar aprendizado,
                clareza e potencial.
              </p>
            </div>
              ...
              {evolutionTracks.map((track) => (
                ...
                >
                  <div className="mt-4 space-y-3 text-sm text-slate-700">
                    <p>
                      <strong>Técnicas:</strong> {track.technical.join(", ")}
                    </p>
                    <p>
                      <strong>Soft skills:</strong> {track.soft.join(", ")}
                    </p>
                    <p>
                      <strong>Tempo médio:</strong> {track.time}
                    </p>
                    <p>
                      <strong>Sinal de prontidão:</strong> {track.ready}
                    </p>
                    <p>
                      <strong>Como acelerar:</strong> {track.accelerate}
                    </p>
                  </div>
                </DetailsChevronOnly>
              ))}
          ...
          <div>
            <div className="mb-5">
              ...
              <h2 className="font-display text-3xl font-black text-slate-950">
                Dicas, vídeos e sugestões por caminho
              </h2>
              <p className="mt-2 max-w-3xl text-sm font-semibold text-slate-600">
                Escolha a área que você quer seguir e use os projetos, dicas e
                vídeos como guia para evoluir com foco.
              </p>
            </div>
              ...
              {careerEvolutionByArea.map((item) => (
                ...
                >
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                      <h4 className="mb-2 font-display text-lg font-black text-slate-950">
                        Próximos passos
                      </h4>
                      <ul className="space-y-2 text-sm text-slate-700">
          ...
          <div className="card-brutal overflow-x-auto rounded-2xl bg-white">
            <table className="w-full min-w-[760px] text-sm">
              <thead className={cn(ac.tableBanner)}>
                <tr>
                  <th className="p-3 text-left">Certificação</th>
                  <th className="p-3 text-left">Área</th>
                  <th className="p-3 text-left">Dificuldade</th>
                  <th className="p-3 text-left">Custo</th>
                  <th className="p-3 text-left">Impacto</th>
                  <th className="p-3 text-left">Vale a pena?</th>
                </tr>
              </thead>
              <tbody>
                {certifications.map((cert) => (
                  <tr key={cert.name} className="border-t">
                    <td className="p-3 font-bold">{cert.name}</td>
                    <td className="p-3">{cert.area}</td>
                    <td className="p-3">{cert.difficulty}</td>
                    <td className="p-3">{cert.cost}</td>
                    <td className="p-3">{cert.impact}</td>
                    <td className="p-3">
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-black ${cert.worth ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}
                      >
                        {cert.worth ? "Sim" : "Depende"}
                      </span>
                    </td>
                  </tr>
                ))}
          ...
          <div className="card-brutal rounded-2xl bg-white p-6">
            <h2 className="font-display text-2xl font-black">
              Guia de trabalho no exterior
            </h2>
            {[
              "Plataformas que contratam devs brasileiros remotamente",
              "Nível de inglês necessário por área",
              "Como receber em dólar/euro no Brasil",
              "Como adaptar o currículo para o mercado internacional",
            ].map((item) => (
              <DetailsChevronOnly
                key={item}
              ...
              >
                <p className="mt-2 text-sm text-slate-600">
                  Comece com inglês técnico, portfólio em inglês e plataformas
                  como Toptal, Turing, Andela, Remote.com e Gun.io.
                </p>
              </DetailsChevronOnly>
            ))}
          </div>

...
export default function Tecnologias() {
  ...
  useEffect(() => {
    getTechnologies().then(setTechnologyItems).catch(() => setTechnologyItems(technologies));
  }, []);
  ...
  const filtered = useMemo(
    () =>
      technologyItems.filter((technology) => {
        ...
        const matchesQuery = technology.name.toLowerCase().includes(query.toLowerCase());
    ...
    <Layout>
      ...
      <section className={cn("sticky top-16 z-40 border-b-2 py-4 backdrop-blur", ac.stickyBar)}>
        <div className="container space-y-4">
          <FilterPills accent="violet" options={technologyCategories} value={category} onChange={setCategory} />
          <div className="relative max-w-xl">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
            <input
              className={cn("w-full rounded-lg border-2 bg-white py-3 pl-10 pr-4 text-sm font-bold outline-none", ac.input)}
          ...
          {filtered.map((technology) => (
            ...
            >
              <div className="mb-4 flex items-start justify-between gap-3">
                ...
                />
                <span className={cn("rounded-full px-2 py-1 text-xs font-black", ac.tag)}>{technology.category}</span>
              </div>
              <h2 className="font-display text-xl font-black text-slate-950">{technology.name}</h2>
              ...
              <div className="mt-4">
                <p className="text-xs font-black uppercase tracking-wide text-slate-500">
                  {technology.games?.length ? "Empresas e jogos" : "Empresas/produtos que usam"}
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {[...technology.companies.slice(0, 2), ...(technology.games || []).slice(0, 1)].map((example) => (
                    <span key={example} className={cn("rounded-full px-2 py-1 text-xs font-bold", ac.tag)}>
                      {example}
                    </span>
                  ))}
                </div>
              </div>
              <span className={cn("mt-5 inline-flex items-center gap-2 text-sm font-black group-hover:underline", ac.link)}>
                Ver detalhes <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" aria-hidden />
              </span>
            </Link>
          ))}
        </div>

...
export default function Dicionario() {
  ...
  const filtered = dictionaryTerms.filter((item) => {
    const searchableText =
      `${item.term} ${item.category} ${item.tags.join(" ")} ${item.meaning}`.toLowerCase();
    ...
    <Layout>
      ...
      <section className="relative overflow-hidden border-b-2 border-slate-900 bg-cyan-100 py-12">
        ...
        <div className="container relative">
          <p className="mb-4 inline-flex rounded-full border-2 border-slate-900 bg-cyan-300 px-3 py-1 text-xs font-black uppercase text-slate-950 shadow-[3px_3px_0_#0f172a]">
            dicionário técnico
          </p>
          ...
          <p className="mt-3 max-w-2xl text-slate-950">
            Busque palavras que aparecem em cursos, vagas, roadmaps e conversas
            de tecnologia.
          </p>
        </div>
      </section>
      ...
      <section className="bg-[#ecfeff] py-12">
        <div className="container">
          <p className="mb-6 text-sm text-slate-500">
            {filtered.length} termo{filtered.length !== 1 ? "s" : ""} encontrado
            {filtered.length !== 1 ? "s" : ""}
          </p>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((item) => (
              <div
                key={item.term}
                className="card-invite rounded-2xl bg-white p-5 shadow-[4px_4px_0_#67e8f9]"
              >

    ...
    <Layout>
      ...
      <section className={cn(ac.contentBg, "py-12")}>
        <div className="container space-y-10">
          {!isPro ? (
            <ProGate description="Gere mensagens personalizadas para recrutadores, devs sênior, pessoas que admira e pedidos de indicação." />
          ) : (
            <AiToolPanel
              endpoint="networking-message"
              accent="emerald"
              title="Gerador de mensagem de networking com IA"
              description="Receba três versões: direta, descontraída e formal."
              fields={[
                { name: "contactType", label: "Tipo de contato" },
                { name: "name", label: "Nome e cargo da pessoa" },
                { name: "company", label: "Empresa da pessoa" },
                { name: "goal", label: "Objetivo da mensagem" },
            ...
            {networkingGuide.map((item) => (
              ...
              >
                <p className="mt-3 text-sm text-slate-600">
                  Seja específico, respeitoso e deixe claro que você valoriza o
                  tempo da outra pessoa.
                </p>
              </DetailsChevronOnly>
            ))}
          </div>
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {communityMap.map((area) => (
              <article
                key={area.area}
                className="card-brutal rounded-2xl bg-white p-5"
              >
                <h3 className="font-display text-xl font-black">{area.area}</h3>

...
// HTML prerenderizado / rich results.
const cursosItemList = {
  mainEntity: {
    "@type": "ItemList",
    numberOfItems: Math.min(cursosGratuitos.length, FREE_COURSES_SAMPLE_SIZE),
    itemListElement: cursosGratuitos
      .slice(0, FREE_COURSES_SAMPLE_SIZE)
      .map((curso, index) => ({
        "@type": "ListItem",
        position: index + 1,
        item: {
    ...
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-1 rounded-full border-2 border-slate-900 bg-white px-2.5 py-1 text-xs font-black text-slate-900 shadow-[2px_2px_0_var(--bnt-shadow)] transition-all motion-safe:hover:-translate-y-px focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-200"
        >
      ...
      <div className="flex flex-wrap gap-2">
        {atual.opcoes.map((op) => (
          <button
            ...
            className="rounded-full border-2 border-slate-900 bg-white px-4 py-2 text-sm font-bold text-slate-900 shadow-[2px_2px_0_var(--bnt-shadow)] transition-all hover:bg-amber-50 motion-safe:hover:-translate-y-px focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-200"
          >
            {op.label}
          </button>
        ))}
...
// Card de curso parceiro (influencer). Pinado como 1o item da grade, fora do
// pipeline de filtro/sort e do gate free/Pro. Segue o padrao visual claro dos
// demais cards de curso; o selo "Parceiro" justifica o pin.
function ParceiroCard({ parceiro }: { parceiro: CursoParceiro }) {
  return (
    <div className="group flex flex-col rounded-2xl border-2 border-slate-900 bg-white p-6 shadow-[5px_5px_0_#fbbf24] transition-all duration-200">
      <div className="mb-4 flex items-start justify-between gap-3">
        <span className="inline-flex items-center gap-1.5 rounded-md border border-violet-300 bg-violet-50 px-2.5 py-1 text-[11px] font-black text-violet-800">
...
export default function Cursos() {
  ...
  // Fonte canonica e estatica (data.ts). O gate e no que RENDERIZA, nao de onde
  // vem: free so ve a amostra; o resto nunca entra no DOM. A rota /api/content/
  // courses e fatiada por tier no servidor (defesa pra chamada direta).
  const courses = cursosGratuitos;
  ...
  const visibleCourses = isPro
    ? filtered
    : filtered.filter((c) => freeCourseIds.has(c.id));

...
- **Guard cuja cobertura é lista manual é guard que expira, e ele sempre falha PASSANDO.** Três instâncias nesta base: a migration que dependia de alguém lembrar de aplicar, o `checkMigrationsApplied` cujo regex cobria 38 de 72 tabelas, e o pre-commit com lista de arquivos escrita à mão que liberou árvore vermelha. Todo instrumento de verificação enumera a cobertura da FONTE DE VERDADE (o código, as migrations, o catálogo) e falha ao encontrar algo que não conhece. Item que não é para cobrir entra numa lista de exceção declarada, com motivo, nunca por omissão. Referência: `scripts/mutateLinkedinThresholds.mjs`, que varre os arquivos e aborta em sítio numérico não classificado.

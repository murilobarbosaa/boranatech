...
- **Instrumento de verificação cujo escopo é derivado por um parser que pode sub-casar em silêncio sempre falha PASSANDO.** Lista escrita à mão é só o caso degenerado; regex, janela de contexto e casamento de padrão são o mesmo mecanismo. Sete instâncias nesta base: a migration que dependia de alguém lembrar de aplicar; o regex do `checkMigrationsApplied` que enxergava 38 de 72 tabelas; o pre-commit com lista de arquivos, que liberou árvore vermelha; a janela de 4000 caracteres que classificou duas RPC reais como trigger e as tirou da verificação; o `stripSqlComments` que casou o `/*` de `/api/cron/*` com o `*/` de um cron `*/6` e apagou 3663 caracteres de SQL real; o parser que aplicava todos os `CREATE` antes de todos os `DROP`, desfazendo declaração de quem dropa e recria; e o `env -i`, que para provar "a suíte roda sem ambiente" limpou as variáveis do shell em vez do arquivo `.env` do disco, que o `dotenv` lê direto, e devolveu 549 testes verdes sobre uma condição que nunca existiu. Nenhum deles acusou nada: todos reportaram sucesso sobre uma superfície menor.

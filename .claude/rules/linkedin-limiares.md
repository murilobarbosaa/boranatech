...
- **Sitio numerico novo em arquivo do `FONTES` precisa ser classificado NO MESMO COMMIT**, em `MUT` (e um limiar, entao e mutado e tem de quebrar teste) ou em `NAO_LIMIAR` com o motivo. Quais arquivos sao esses esta logo abaixo, e a fonte da verdade e o script, nao esta lista.
...
- **O `FONTES` do script E A LISTA CANONICA DE ESCOPO; o `paths` deste frontmatter e uma COPIA** e precisa acompanhar o `FONTES` quando ele mudar. Sao dois arquivos que descrevem a mesma coisa e nada os compara automaticamente, entao a copia envelhece calada.
- **Em 2026-08-18 tres sitios orfaos nasceram em `server/lib/linkedinAnalyze.ts` exatamente por causa dessa divergencia**: o arquivo estava no `FONTES` e fora deste `paths`, entao esta regra nao carregou ao editar aquele arquivo, os sitios entraram sem classificacao e a `main` ficou vermelha. O `paths` foi alinhado no mesmo dia.
- ATENCAO ao unico item do `paths` que NAO esta no `FONTES`, o diretorio de componentes (`client/src/components/linkedin`). A assimetria e deliberada agora que esta escrita: a regra carrega ali para dar contexto, mas o `pnpm check:limiares` NAO audita aquele diretorio, entao sitio numerico novo em componente de LinkedIn nao e pego pelo guard. Se isso passar a importar, o conserto e acrescentar o arquivo ao `FONTES`, nao confiar nesta linha.

...
export const DadosPessoaisSchema = z.object({
  nome: z
    .string()
    .min(1)
    .describe("Nome completo da pessoa, vindo do cadastro."),
  email: z.string().email().describe("Email principal, vindo do cadastro."),
  telefone: z
    .string()
    .nullable()
    .describe("Telefone com DDD se a pessoa forneceu, senão null."),
...
export const ObjetivoSchema = z.object({
  cargo: z
    ...
    .min(1)
    .describe(
      "Cargo alvo (ex: Desenvolvedor Backend Júnior, Senior Software Engineer).",
    ),
  area: z
    .string()
    .min(1)
    .describe("Área tech (ex: backend, frontend, dados, design, devops)."),
  nivel: z
    .string()
    .min(1)
    .describe("Nível buscado (estágio, trainee, júnior, pleno, sênior, lead)."),
});
...
export const FormacaoItemSchema = z.object({
  curso: z
    .string()
    .min(1)
    .describe("Nome do curso ou bootcamp. Identificador essencial do item."),
  instituicao: z
    .string()
    .nullable()
    .describe(
      "Instituição se a pessoa forneceu, senão null. Cursos online sem instituição formal podem ficar null.",
    ),
  periodo: z
    .string()
    .nullable()
    .describe(
      "Período (ex: '2023 - 2025', '2024', 'em andamento desde 2023') se a pessoa forneceu, senão null.",
    ),
  status: z
    .string()
    .nullable()
    .describe("Status como 'concluído', 'em andamento', 'trancado', ou null."),
});
...
export const ExperienciaItemSchema = z.object({
  empresa: z
    ...
    .min(1)
    .describe(
      "Nome da empresa, projeto ou atividade. Identificador essencial do item.",
    ),
  cargo: z
    .string()
    .min(1)
    .describe("Cargo ou função exercida. Identificador essencial do item."),
  periodo: z
    .string()
    .nullable()
    .describe(
      "Período (ex: 'jan/2024 até hoje', '2022 - 2024') se a pessoa forneceu, senão null.",
    ),
  responsabilidades: z
    .array(z.string())
    .describe(
      "Bullets de responsabilidades em verbos de ação. Vazio se nada foi dito.",
    ),
  conquistas: z
    .array(z.string())
    .describe(
      "Bullets de conquistas com quantificação quando possível. Vazio se nada foi dito.",
    ),
});
...
export const ProjetoItemSchema = z.object({
  nome: z
    .string()
    .min(1)
    .describe("Nome do projeto. Identificador essencial do item."),
  descricao: z
    .string()
    .nullable()
    .describe(
      "Descrição curta do que o projeto faz, ou null se a pessoa só citou o nome.",
    ),
  tecnologias: z.array(z.string()).describe("Lista de tecnologias usadas."),
  link: z
    .string()
    .nullable()
    .describe("Link do projeto (repo, deploy, etc) ou null."),
});
...
export const IdiomaItemSchema = z.object({
  idioma: z
    .string()
    .min(1)
    .describe("Idioma (ex: Português, Inglês, Espanhol)."),
  nivel: z
    .string()
    .min(1)
    .describe("Nível (básico, intermediário, avançado, fluente, nativo)."),
});
...
export const CertificacaoItemSchema = z.object({
  nome: z
    .string()
    .min(1)
    .describe("Nome da certificação. Identificador essencial do item."),
  instituicao: z
    .string()
    .nullable()
    .describe(
      "Quem emitiu se a pessoa forneceu, senão null. Certificações informais podem ficar null.",
    ),
...
export const CurriculoSchema = z.object({
  ...
  formato: z
    .enum(FORMATOS)
    .describe("Formato escolhido (hibrido, cronologico, harvard)."),
  persona: z
    .enum(PERSONAS)
    .describe("Persona detectada (estudante, transicao, junior, experiente)."),
  resumoProfissional: z
    .string()
    .min(1)
    .describe(
      "Parágrafo curto de 2 a 4 frases sintetizando objetivo, principais habilidades e diferencial.",
    ),
  formacao: z
    .array(FormacaoItemSchema)
    .describe("Itens de formação. Vazio se nada foi mencionado."),
  experiencias: z
  ...
  projetos: z
    .array(ProjetoItemSchema)
    .describe("Projetos pessoais ou acadêmicos."),
  habilidades: z
    .array(z.string())
    .describe("Lista de tecnologias e ferramentas mencionadas."),
  idiomas: z
    .array(IdiomaItemSchema)
    .describe("Idiomas falados com nível. Vazio se só português."),
  certificacoes: z

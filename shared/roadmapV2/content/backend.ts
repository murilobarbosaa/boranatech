...
export const backend: RoadmapV2 = {
  ...
  sections: [
    ...
    // TODO(Ana): etapas novas (aprofundamento), revisar copy.
    {
      id: "arquitetura",
      title: "Arquitetura e organização do código",
      description:
        "Como estruturar um back-end que cresce sem virar um nó: separar responsabilidades e organizar o projeto.",
      level: "intermediario",
      children: [
        {
          ...
          description:
            "Dividir o código por responsabilidade: entrada, regra de negócio e acesso a dados.",
          content:
            "Um back-end que cresce sem organização vira o famoso emaranhado onde tudo depende de tudo e nenhuma mudança é segura. A primeira defesa contra isso é separar o código em camadas por responsabilidade. Um arranjo comum tem três: uma camada que recebe as requisições (as rotas e controladores), uma que contém as regras de negócio (o que o sistema realmente faz) e uma que cuida do acesso aos dados (conversar com o banco).\n\nA regra é que cada camada só conhece a vizinha e conversa por contratos claros. O controlador não sabe montar uma consulta ao banco; ele chama a regra de negócio, que chama a camada de dados. Assim você troca o banco sem mexer nas regras, e testa as regras sem precisar de um banco de verdade.\n\nNo começo essa separação parece burocracia para um sistema pequeno, e às vezes é. Mas conforme o projeto cresce, ela é o que mantém o código compreensível e mudável. Aprender a reconhecer o que é entrada, o que é regra e o que é dado treina o olho para organizar qualquer back-end.",
        },
        {
          id: "arquitetura.organizacao",
          title: "Organizando pastas e módulos",
          description:
            "Estruturar os arquivos por funcionalidade, de forma que qualquer um ache o que procura.",
          content:
            "Além das camadas, existe a questão prática de como distribuir os arquivos em pastas. Duas abordagens comuns: organizar por tipo técnico (todas as rotas juntas, todos os modelos juntos) ou por funcionalidade (tudo de usuários numa pasta, tudo de pedidos em outra). Para projetos que crescem, organizar por funcionalidade costuma escalar melhor, porque mantém junto o que muda junto.\n\nO objetivo é previsibilidade: alguém novo no projeto deve conseguir adivinhar onde está cada coisa sem perguntar. Nomes claros de pastas e arquivos, uma convenção seguida em todo o projeto e evitar arquivos gigantes que fazem de tudo são hábitos que pagam dividendos a cada mês que o código vive.\n\nNão existe estrutura perfeita universal, e times gastam energia demais discutindo isso. O importante é escolher uma convenção sensata e ser consistente. Coerência vale mais que perfeição: um projeto organizado de forma razoável e uniforme é muito mais agradável de manter do que um cheio de exceções espertas.",
        },
      ],
    },
    // TODO(Ana): etapa nova (aprofundamento), revisar copy.
    {
      ...
      description:
        "Fazer o back-end responder rápido mesmo sob carga: entender gargalos e usar cache com cuidado.",
      level: "intermediario",
      children: [
        {
          id: "performance.cache",
          title: "O que é cache e quando usar",
          description:
            "Guardar resultados prontos para não recalcular ou rebuscar a mesma coisa toda vez.",
          content:
            "Cache é guardar o resultado de algo caro para reaproveitá-lo depois, em vez de refazer o trabalho toda vez. Se uma consulta ao banco é lenta e a resposta muda pouco, guardar o resultado por alguns minutos e servi-lo direto da memória pode transformar uma resposta de segundos numa de milissegundos.\n\nO cache aparece em várias camadas: dentro da aplicação, num serviço de cache dedicado e rápido, ou até nas respostas HTTP. A ideia é sempre a mesma: evitar refazer trabalho já feito. Para dados muito acessados e que mudam pouco, o ganho é enorme.\n\nO desafio do cache não é guardá-lo, e sim invalidá-lo: saber quando o dado guardado ficou velho e precisa ser atualizado. Servir um dado desatualizado pode ser pior que servir devagar. Por isso a regra de ouro é começar sem cache, medir onde realmente dói, e cachear com uma estratégia clara de expiração, não por todo lado por precaução.",
        },
        {
          id: "performance.gargalos",
          title: "Gargalos e otimização de consultas",
          description:
            "Achar o que está lento de verdade antes de otimizar, e por que o banco costuma ser o suspeito.",
          content:
            "Otimizar sem medir é chutar no escuro, e quase sempre no lugar errado. A disciplina correta é primeiro descobrir o gargalo real: onde o tempo está sendo gasto de fato. Muitas vezes a intuição engana, e o que parecia o problema não é. Ferramentas de medição e logs de tempo apontam onde vale investir.\n\nNo back-end, o suspeito número um costuma ser o banco de dados. Consultas mal escritas, a ausência de um índice que aceleraria uma busca, ou o padrão de fazer muitas consultas pequenas em vez de uma bem pensada derrubam a performance mais do que qualquer outra coisa. Aprender a ler o que uma consulta está fazendo por baixo é uma habilidade de alto retorno.\n\nO princípio geral é o mesmo de sempre: a solução mais simples primeiro, medir antes de otimizar, e resolver o maior gargalo antes de se preocupar com os pequenos. Código rápido que ninguém precisava é esforço desperdiçado; código rápido no ponto que travava é o que faz o sistema aguentar crescer.",
        },
      ],
    },
    // TODO(Ana): etapa nova (aprofundamento), revisar copy.
    {
      ...
      description:
        "Tirar o trabalho pesado do caminho da resposta: processar em segundo plano com filas e workers.",
      level: "avancado",
      children: [
        {
          id: "filas.async",
          title: "Por que processar em segundo plano",
          description:
            "Nem tudo precisa acontecer enquanto o usuário espera; algumas tarefas devem sair do caminho.",
          content:
            "Quando um usuário faz uma requisição, ele espera uma resposta rápida. Mas algumas tarefas são demoradas: enviar um e-mail, gerar um relatório pesado, processar uma imagem, chamar um serviço externo lento. Fazer isso enquanto o usuário espera trava a resposta e piora a experiência.\n\nA solução é o processamento assíncrono: em vez de fazer a tarefa demorada na hora, o sistema apenas registra que ela precisa ser feita e responde ao usuário imediatamente. A tarefa real acontece depois, em segundo plano, sem ninguém esperando na frente da tela. O usuário recebe o retorno de que o pedido foi aceito, e o trabalho pesado corre por fora.\n\nEsse padrão é essencial para sistemas que fazem tarefas caras. Ele separa o que precisa ser imediato (confirmar que o pedido chegou) do que pode levar seu tempo (executar o pedido de fato), deixando a aplicação responsiva mesmo quando há trabalho pesado acontecendo.",
        },
        {
          id: "filas.workers",
          title: "Filas de mensagens e workers",
          description:
            "Uma fila guarda as tarefas pendentes; workers as consomem e processam no seu ritmo.",
          content:
            "A forma clássica de implementar processamento assíncrono é com uma fila. A aplicação coloca a tarefa a fazer numa fila (uma lista de trabalhos pendentes), e responde ao usuário. Do outro lado, processos chamados workers ficam pegando tarefas da fila e executando, um a um, no ritmo que conseguem.\n\nEsse desenho traz vantagens além da rapidez da resposta. Se chega um pico de trabalho, a fila absorve o excesso e os workers vão dando conta aos poucos, em vez de o sistema desabar. Se um worker falha no meio de uma tarefa, ela pode voltar para a fila e ser tentada de novo. E dá para adicionar mais workers quando o volume cresce, escalando o processamento.\n\nExistem ferramentas dedicadas a filas de mensagens que cuidam dessa mecânica de forma robusta. Para começar, o importante é entender o conceito: produtor coloca na fila, fila guarda, worker consome. Esse trio é a base de praticamente todo processamento em segundo plano que você vai encontrar em sistemas reais.",
        },
      ],
    },
    {

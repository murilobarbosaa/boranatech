...
--
-- COMO RODAR: os `select` de conferencia ficam fora da transacao, um de cada
-- vez; as duas escritas vao juntas num unico bloco `begin ... commit`.
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- (1) AS DUAS LINHAS, numa transacao so. AFETA: 1 + 1.
-- -----------------------------------------------------------------------------
-- OS DOIS `update` VAO JUNTOS OU NAO VAO. As duas linhas sao uma ocorrencia so,
-- e a secao acima ja diz por que metade aplicada e PIOR que nenhuma: com o
-- estorno preenchido e a cobranca nao, "receita por plano" mostraria o anual com
-- -R$ 222,00 e nenhuma receita correspondente. `begin ... commit` transforma
-- isso de "cuidado ao rodar" em impossivel.
--
-- O SQL Editor do Supabase executa o texto colado INTEIRO numa requisicao, na
-- mesma sessao, entao o bloco abrange de fato os dois comandos.
--
...
-- bloco (0). O `and plan_code is null` torna cada `update` idempotente e impede
-- sobrescrever um valor que alguem tenha preenchido no meio tempo.
--
-- A COBRANCA VEM PRIMEIRO, pelo motivo da secao "A JANELA DO SYNC" acima.
...
-- -----------------------------------------------------------------------------
-- (2) DEPOIS. ESPERADO: 2 linhas, ambas 'pro_annual', ambas com user_id NULL
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- (3) CONFERENCIA AMPLA. ESPERADO: exatamente 2 linhas, e NAO zero
-- -----------------------------------------------------------------------------
...
--
-- Depois de rodar o bloco (1), esta consulta tem de devolver EXATAMENTE essas
-- duas. Se devolver tres ou quatro, a transacao nao foi comitada. Se devolver
-- uma, algo alem deste script escreveu no meio tempo.

...
import { getPlanChargeValue } from "../../shared/planPricing";
...
async function createCheckout(
  ...
  if (!profile) {
    throw createError(404, "not_found", "Perfil não encontrado.");
  }
  ...
  const customer = await getOrCreateAsaasCustomer({
    userId,
    name: profile.name || profile.email || input.user.email,
    email: profile.email || input.user.email,
  });
...
async function handleWebhook(input: WebhookInput): Promise<WebhookResult> {
  ...
  if (typeof received !== "string" || !received) {
    throw createError(401, "unauthorized", "Header asaas-access-token ausente.");
  }
  ...
  if (!crypto.timingSafeEqual(receivedHash, expectedHash)) {
    throw createError(401, "unauthorized", "Token do webhook inválido.");
  }
  ...
  try {
    ...
    if (
      ...
      return { received: true, out_of_order: true };
    }

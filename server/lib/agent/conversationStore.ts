...
export async function createConversation(
  ...
  try {
    const insertRow: { user_id: string; title?: string } = { user_id: userId };
    // title so entra se vier nao vazio; do contrario fica NULO (default da coluna).
    ...
    // O store nunca lanca por contrato: excecao (rede, timeout, tabela ausente
    // ate a migration ser aplicada) vira ok:false, nao propaga.
    console.warn("[agent/store] createConversation lancou:", err);
    return { ok: false, error: "create_failed" };
  }
}
...
export async function listConversations(
  ...
  try {
    ...
    console.warn("[agent/store] listConversations lancou:", err);
    return { ok: false, error: "list_failed" };
  }
}
...
export async function getConversation(
  ...
  try {
    ...
    console.warn("[agent/store] getConversation lancou:", err);
    return { ok: false, error: "get_failed" };
  }
}
...
export async function appendMessage(
  ...
  try {
    ...
    console.warn("[agent/store] appendMessage lancou:", err);
    return { ok: false, error: "append_failed" };
  }
}
...
export async function deleteConversation(
  ...
  try {
    ...
    console.warn("[agent/store] deleteConversation lancou:", err);
    return { ok: false, error: "delete_failed" };
  }
}

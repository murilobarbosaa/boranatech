...
export const getResumeAnalysis: AgentTool = {
  name: "get_resume_analysis",
  tier: "pro",
  description:
    "Retorna a analise de curriculo mais recente do proprio usuario (nota, faixa, cargo alvo, data). O resultado fica salvo, entao retorna o resultado de fato. Recurso do Plano Pro. A identidade vem do contexto seguro do servidor, nunca de argumentos.",
  parameters: {
    type: "object",
    properties: {},
    required: [],
  ...
  async execute(_args, ctx) {
    ...
    if (error) {
      console.warn("[agent/get_resume_analysis] query falhou:", error.message);
      ...
      return JSON.stringify({
        ok: false,
        message:
          "Nao foi possivel consultar a analise de curriculo agora. Nao invente; sugira tentar de novo.",
      });
    }

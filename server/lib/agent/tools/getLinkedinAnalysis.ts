...
export const getLinkedinAnalysis: AgentTool = {
  ...
  tier: "pro",
  description:
    "Retorna a analise de LinkedIn mais recente do proprio usuario (area, nivel, nota, faixa, data). Esta e a unica ferramenta cujo resultado fica salvo, entao retorna o resultado de fato. Recurso do Plano Pro. A identidade vem do contexto seguro do servidor, nunca de argumentos.",
  parameters: {
    type: "object",
    properties: {},
    required: [],
  ...
  async execute(_args, ctx) {
    ...
    if (error) {
      console.warn("[agent/get_linkedin_analysis] query falhou:", error.message);

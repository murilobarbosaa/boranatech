...
describe("contagem de violacoes de lastro", () => {
  ...
  it("o resto do evento nao mudou: nivel, tags e fingerprint por tipo", () => {
    // O contexto carrega um MARCADOR de propósito. Ele e o que torna a
    // assercao de redacao discriminante: sem um token improvavel no texto,
    // "nao vazou" seria indistinguivel de "o payload nao tem texto nenhum".
    const MARCADOR = "ZQXJSOBREZQXJ";
    const comMarcador: Violacao = {
      tipo: "bullet_sem_origem",
      campo: "headlines",
      contexto: `Dev | Node ${MARCADOR}`,
      termo: "3 bullet(s)",
    };
    registrarViolacao(comMarcador);
    ...
    // CONTRATO DE REDACAO da Fase 4, e nao o campo cru de antes. `contexto`
    // saia inteiro no `extra`, e o `extra` do Sentry viaja para fora tanto
    // quanto o stdout; `violacaoParaLog` o substituiu pelo TAMANHO. A assercao
    // afirma os dois lados do contrato, porque um so nao prova nada: a contagem
    // ESTA presente, e o texto NAO esta em lugar nenhum do payload capturado.
    expect(o.extra.contextoChars).toBe(comMarcador.contexto.length);
    expect(JSON.stringify(capturados[0])).not.toContain(MARCADOR);
  });

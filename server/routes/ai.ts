  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!toolConfig) {
      ...
      return next(
        createError(
          404,
          "not_found",
          `Ferramenta '${toolKey}' não encontrada.`,
        ),
      );
    }
    ...
    if (toolConfig.requiresPro && !req.isPro) {
      ...
      return next(
        createError(
          403,
          "forbidden",
          "Plano Pro necessário para usar esta ferramenta.",
        ),
      );
    }
    ...
    if (toolConfig.injectLoginContext && req.user) {
      try {
        ...
        console.warn(
          "[ai] Falha ao montar loginContext, seguindo sem ele:",
          err,
        );
      }
    }
      ...
      if (cleaned.length === 0) {
        ...
        return next(
          createError(
            400,
            "invalid_request",
            "Envie pelo menos uma mensagem válida.",
          ),
        );
      }
    ...
    if (inputChars > toolConfig.maxInputChars) {
      ...
      return next(
        createError(
          413,
          "payload_too_large",
          `Input muito grande. Máximo: ${toolConfig.maxInputChars} caracteres.`,
        ),
      );
    }
    ...
    if (!env.openaiApiKey) {
      await logUsage({
        ...
        requestId,
        status: "error",
        errorMessage: "OpenAI key missing",
        inputChars,
        model: toolConfig.model,
      });
      return next(
        createError(503, "upstream_error", "Serviço de IA não configurado."),
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        await logUsage({
          ...
          requestId,
          status: "error",
          errorMessage: `OpenAI ${response.status}`,
          inputChars,
          model: toolConfig.model,
        });
        return next(
      ...
      const result =
        data.choices?.[0]?.message?.content || "A IA não retornou conteúdo.";
      ...
      if (toolConfig.responseFormat) {
        ...
        try {
          ...
          console.error(
            "[ai] JSON parse falhou pra tool com responseFormat:",
            toolKey,
            parseErr,
            "raw:",
            result.slice(0, 500),
        ...
        const validation =
          toolConfig.responseFormat.zodSchema.safeParse(parsedJson);
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (toolConfig.responseFormat) {
      return next(
        createError(
          ...
          "Esta ferramenta usa saída estruturada e não suporta streaming.",
        ),
      );
    }
    ...
    if (toolConfig.injectLoginContext && req.user) {
      try {
        ...
        console.warn(
          "[ai/stream] Falha ao montar loginContext, seguindo sem ele:",
          err,
        );
      }
    }
    ...
    try {
      ...
      res.write(
        `data: ${JSON.stringify({ error: "Erro de conexão com o serviço de IA." })}\n\n`,
      );
      res.write(`data: [DONE]\n\n`);
      res.end();
      return;
    ...
    if (!openaiResponse.ok || !openaiResponse.body) {
      ...
      await logUsage({
        ...
        requestId,
        status: "error",
        errorMessage: `OpenAI ${openaiResponse.status}`,
        inputChars,
        model: toolConfig.model,
      });
      res.write(
        `data: ${JSON.stringify({ error: "Erro ao processar com IA. Tente novamente." })}\n\n`,
      );
      res.write(`data: [DONE]\n\n`);
      res.end();
      return;
